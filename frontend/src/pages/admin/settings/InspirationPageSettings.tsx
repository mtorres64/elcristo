...
import type { InspirationProject, InspirationSettings } from "../../../types/content";
...
export function emptyProject(): InspirationProject {
  return { id: crypto.randomUUID(), title: "", description: "", location: "", image: "" };
}

/** Estado de la página "Inspiración" vive en `Settings.tsx` (el padre),
 * igual que el carrusel del hero y "Sobre Nosotros" — así el botón
 * "Guardar cambios" puede salir tanto en la barra desktop de acá como en
 * la sticky bar mobile. */
export function InspirationPageSettings({
  data,
  loading,
  saving,
  onChange,
  ...
  data: InspirationSettings;
  loading: boolean;
  saving: boolean;
  onChange: (patch: Partial<InspirationSettings>) => void;
  onSave: () => void;
}) {
  async function uploadHeroImage(file: File) {
    try {
      const url = await contentService.uploadInspirationImage(file);
  ...
  function patchProject(index: number, patch: Partial<InspirationProject>) {
    onChange({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="hidden sm:flex items-center justify-between bg-white border border-[#E8E2D8] rounded-lg px-5 py-4">
        <div>
          <p className="text-sm font-semibold text-[#1A1A1A]">Página "Inspiración"</p>
          <p className="text-xs text-[#8A8A8A] mt-0.5">
            Proyectos de paisajismo de la sección /inspiracion
          </p>
        </div>
        <button
          onClick={onSave}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6">
          ...
          <FormField label="Título">
            <input
              ...
              className={INPUT}
              placeholder="Proyectos que inspiran."
            />
          </FormField>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-4">
          <FormField label="Título de la sección">
            <input
              ...
              className={INPUT}
              placeholder="Paisajismo hecho a medida"
            />
          </FormField>
          <FormField label="Texto">
            <textarea
              ...
              placeholder="Presentá brevemente los proyectos que van a mostrarse…"
            />
          </FormField>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#E8E2D8] rounded-lg p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-sm font-semibold text-[#1A1A1A]">Proyectos</p>
            <p className="text-xs text-[#8A8A8A] mt-0.5">
              Se muestran en este orden, cada uno con su foto a ancho completo
            </p>
          </div>
          <button
            onClick={addProject}
        ...
        {data.projects.length === 0 && (
          <p className="text-sm text-[#6B6B6B] py-6 text-center">Todavía no hay proyectos cargados.</p>
        )}
...
function ProjectCard({
  ...
  index: number;
  total: number;
  onChange: (patch: Partial<InspirationProject>) => void;
  onRemove: () => void;
  onMove: (dir: -1 | 1) => void;
}) {
    ...
    <div className="border border-[#E8E2D8] rounded-lg p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-1">
          ...
          <IconButton title="Eliminar proyecto" danger onClick={onRemove}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className="flex flex-col gap-3">
          <FormField label="Título">
            <input
              ...
              className={INPUT}
              placeholder="Jardín mediterráneo"
            />
          </FormField>
          <FormField label="Ubicación / rubro">
            <input
              ...
              className={INPUT}
              placeholder="Casa particular, zona norte"
            />
          </FormField>
          <FormField label="Descripción">
            <textarea
              ...
              placeholder="Contá brevemente en qué consistió el proyecto…"
            />
          </FormField>
        </div>
      </div>
